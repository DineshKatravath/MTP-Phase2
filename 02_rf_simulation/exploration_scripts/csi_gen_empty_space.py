...
from sionna.rt import (load_scene, load_scene_from_string, Transmitter,
                        Receiver, PlanarArray, PathSolver, RadioMaterial)
...
plt.ion()
fig_live, (ax1, ax2) = plt.subplots(2, 1, figsize=(8, 6))
...
def update_plots(H, CIR, frame_name):
    ...
    ax1.plot(np.abs(H))
    ax1.set_title(f"CSI Magnitude - {frame_name}")
...
def generate_video(mesh_paths, output_file="csi_cir_output.mp4", fps=10):
    ...
    fig_vid, (ax1_v, ax2_v) = plt.subplots(2, 1, figsize=(8, 6))
    canvas = FigureCanvasAgg(fig_vid)
    ...
    for idx, mesh_path in enumerate(mesh_paths):
        ...
        fig_vid.suptitle(
            f"mmWave 28 GHz Hand Gesture  —  "
            f"Frame {idx+1} / {total}  "
            f"({'█' * int(20*(idx+1)/total)}{'░' * (20 - int(20*(idx+1)/total))})",
            fontsize=9, y=0.02
        )

        fig_vid.tight_layout(rect=[0, 0.04, 1, 1]) 
...
def main():
    ...
    # video generation
    # generate_video(mesh_paths, output_file="csi_cir_empty_output.mp4", fps=10)
