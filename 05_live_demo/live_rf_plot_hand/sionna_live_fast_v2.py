...
def main():
    ...
    task_queue  = queue.Queue(maxsize=PREFETCH_AHEAD + 2)
    ready_queue = queue.Queue(maxsize=PREFETCH_AHEAD + 2)
    ...
    try:
        while True:
            # ── Poll for new PLY files ────────────────────────────────────
            files = sorted(
                f for f in os.listdir(LIVE_DIR) if f.endswith(".ply")
            )

            if files:
                last_seen_t = time.time()

                # Keep only the latest frame — drop backlog
                fname     = files[-1]
                ...
                # Only process if we haven't seen this filename yet
                if fname not in pending_clean:
                    # CLEAN (fast — trimesh roundtrip, ~5ms)
                    try:
                        clean_frame(candidate, clean_path)
                    except Exception as e:
                        print(f"[clean error] {fname}: {e}")
                        try: os.rename(candidate, candidate + ".err")
                        except: pass
                    ...
                    if not calibrated:
                        try:
                            ...
                            # Normalize first frame synchronously so
                            # TX/RX calibration can happen immediately
                            normalize_frame(clean_path, norm_path, sf)
                            TX_POSITION, RX_POSITION = calibrate_tx_rx(norm_path)
                            calibrated = True
                            print(f"[init] Calibrated. TX={TX_POSITION.round(3)}"
                                  f"  RX={RX_POSITION.round(3)}\n")
                            # Enqueue this frame's norm result directly
                            # (already done synchronously above)
                            verts = np.array(
                                trimesh.load(norm_path, process=False).vertices
                            )
                            centre = verts.mean(axis=0)
                            span   = verts.max() - verts.min()
                            ready_queue.put((fname, norm_path, centre, span))
                        ...
                        # Submit to background norm thread (non-blocking)
                        try:
                            task_queue.put_nowait(
                                (fname, clean_path, norm_path)
                            )
                            pending_clean[fname] = clean_path
                        except queue.Full:
                            # Prefetch queue full — solver is the bottleneck,
                            # skip this frame to avoid unbounded backlog
                            pass

            elif last_seen_t and (time.time() - last_seen_t > IDLE_TIMEOUT):
                print("\nNo new frames — exiting.")
                break
            else:
                time.sleep(POLL_INTERVAL)

            # ── Drain ready_queue — run Sionna on each ready frame ────────
            # Non-blocking: process everything available right now,
            # then go back to polling for new PLY files.
            while True:
                try:
                ...
                if norm_path_r is None:
                    # Normalization failed for this frame
                    idx += 1
                ...
                try:
                    ...
                    print(
                        f"[{idx:04d}] {fname_r}  "
                        f"centre={centre.round(3)}  span={span:.4f}m  "
                        f"RF={elapsed:.2f}s  ({fps:.1f} fr/s)  "
                ...
                if os.path.exists(candidate_orig):
                    shutil.move(
                        candidate_orig,
                        os.path.join(PROCESSED_DIR, fname_r)
                    )
