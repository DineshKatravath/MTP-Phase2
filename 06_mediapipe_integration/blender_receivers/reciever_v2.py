...
def apply_frame(kp):
    ...
    for name, joints in finger_map.items():
        ...
        pb[joints[1]].rotation_euler[0] = curl * s * strength * 0.9

        pb[joints[2]].rotation_mode = 'XYZ'
        pb[joints[2]].rotation_euler[0] = curl * s * strength * 0.7
    ...
    if all(k in kp for k in thumb_chain):
        ...
        pb["thumb_01"].rotation_euler[0] = curl * 5

        pb["thumb_02"].rotation_mode = 'XYZ'
        pb["thumb_02"].rotation_euler[0] = curl * 4
