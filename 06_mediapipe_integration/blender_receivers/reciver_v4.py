...
FINGER_SMOOTH = 0.25   # lower = smoother but more lag (try 0.1–0.4)
...
prev_q = None
prev_normal = None
prev_curls = {}   # smoothed curl value per finger
...
def apply_frame(kp):
    global prev_q, prev_normal, prev_curls
    ...
    for name, joints in finger_map.items():
        ...
        if DEBUG:
            print(f"{name}: smoothed curl = {curl:.3f}")
    ...
    if all(k in kp for k in thumb_chain):
        ...
        if DEBUG:
            print(f"thumb: smoothed curl = {curl:.3f}")
